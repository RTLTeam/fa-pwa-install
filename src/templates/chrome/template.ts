        ...
        <div id="pwa-install-element">
            <div class="install-dialog chrome ${classMap(installDialogClasses())}">
                <div class="dialog-body">
                    ...
                    ${!disableDescription? 
                        html`<hr><div class="description install-description">${installDescription? installDescription: `${'این سایت دارای قابلیت اپلیکیشن می باشد.'} ${'برای تجربه گسترده و دسترسی آسان، آن را روی دستگاه خود نصب کنید.'}`}</div>` 
                        : ''}
                    ${screenshotsAvailable? html`<pwa-gallery .screenshots=${manifest.screenshots}></pwa-gallery>`: ''}
                    <div class="action-buttons">
                        ${screenshotsAvailable? html`<button class="material-button secondary" @click='${toggleGallery}'>${galleryRequested?'کمتر':'بیشتر'}</button>`:''}
                        <button class="material-button primary install" @click='${install}'>نصب</button>
                    </div>
                </div>
            </div>
